                      ...
                      cudaStream_t stream)
{
    ...
    CUDA_CHECK(cudaMalloc(&current_input, batch_size * seq_len * hidden_dim * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&current_output, batch_size * seq_len * hidden_dim * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&residual, batch_size * seq_len * hidden_dim * sizeof(float)));
